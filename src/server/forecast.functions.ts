...
export const generateForecast = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    for (let i = 1; i <= maxDayLoop; i++) {
      ...
      const tag1Hint = i === 1 ? "\n\nWICHTIG: Dieser Eintrag beschreibt AUSSCHLIESSLICH den Zeitraum ab 06:00 Uhr. Die Vornacht (00:00–06:00) wurde bereits im vorherigen Eintrag (\"Heute Abend \const userPrompt = `Standort: ${locationName}. Schreibe einen Fliesstext für ${weekday}, ${formatted} auf Basis dieser Daten:\n${JSON.stringify(day, null, 2)}`;amp; Nacht\") beschrieben und darf hier NICHT erwähnt werden — kein \"in der Nacht\", keine Beschreibung früher Morgenstunden vor 06:00." : "";
