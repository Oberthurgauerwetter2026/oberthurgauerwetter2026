...
function applyRadarToDay(out: any, dayIndex: number, radar: RadarSnapshot | null, settings: any) {
  ...
  if (settings?.radar_enabled === false) return;
  ...
  if (correction.applied && correction.after_precip_mm != null) {
    ...
    else out.precip = { ...out.precip, avg: correction.after_precip_mm };
    if (correction.ratio > 1 && out.precip_prob?.avg != null) {
      ...
      if (boosted > out.precip_prob.avg) out.precip_prob = { ...out.precip_prob, avg: boosted };
    }
  }
}
...
export async function runAutoForecast(creatorId: string | null) {
  ...
  const biasEnabled = (settings as any)?.bias_enabled !== false;
  const biasStations = (((settings as any)?.bias_stations ?? "GUT,STG,TAE") as string)
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    const body = enforceFrostWarning(enforceSkyConsistency(await generateTextNominal(promptTemplate, `Standort: ${locationName}. Schreibe einen Fliesstext für ${weekday}, ${formatted}:\n${JSON.stringify(day, null, 2)}`), day), day);
